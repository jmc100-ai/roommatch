...
async function generateNeighborhoods(city, db, geminiKey, unsplashKey, googlePlacesKey = null, pexelsKey = null, flickrKey = null) {
  ...
  for (const item of items) {
    ...
    if (bbox.lat_min != null) {
      ...
      if (poiCounts) {
        console.log(
          `[neighborhoods] Overpass ${item.name}: cafes=${poiCounts.cafes} restaurants=${poiCounts.restaurants} parks=${poiCounts.parks} shops=${poiCounts.shops} museums=${poiCounts.museums} icon_spots=${poiCounts.icon_spots} trees=${poiCounts.trees} trees_street=${poiCounts.trees_street}`,
        );
      }
    }
