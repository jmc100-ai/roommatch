...
LITEAPI_MAX_RATES_PER_HOTEL — `/hotels/rates` request knob (used by `/api/rates` AND `/api/debug-rates`). Default `100`, clamped 10–250. Higher = more distinct `mappedRoomId` coverage per hotel (especially luxury hotels where one room can carry 8+ rate plans and crowd out the others in the response), at the cost of payload size and parse time. The `[rates] distinct-rooms histogram` log line is the empirical signal for tuning: if most priced hotels are in the 0–1 bucket we're under-fetching; if most are 6+ we may be over-fetching. Mexico City baseline at 100 = ~50K rates / ~25 MB gzipped. Tune to 50 if Render's network struggles, 150 if coverage still feels thin. Above ~200 hits diminishing returns and gets close to LiteAPI's payload ceiling.
```
