...
3. **Detail pass** (when batch size `>= 50`): re-query the top `RATES_DETAIL_TOPN` (default 50) ranked hotels — in **input order, NOT pre-filtered to priced** — in parallel chunks of `RATES_DETAIL_CHUNK` (default 15) each. Reason: LiteAPI has a per-request global rate budget that gets split across the cohort. Big batches → 1 rate per hotel. Small batches with all priced hotels → 2 rates per hotel. Small batches with some unpriced hotels → 3+ rates per priced hotel (the unpriced hotels contribute 0 and "give" their budget share to the priced ones). Validated against `lp3e1a2` in Mexico City — `Superior King Room` (`1178487`) only surfaces when the cohort is **not** pre-filtered to priced hotels. The detail pass merges all 3-5 rates per top hotel into `roomPrices` / `roomNames` / `offerIds` / `roomFreeCancel`. Failure tolerant: `Promise.allSettled` — partial chunk failures just leave those hotels at whatever the main call returned.
