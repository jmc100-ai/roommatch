...
LITEAPI_MAX_RATES_PER_HOTEL — `/hotels/rates` request knob (used by `/api/rates` AND `/api/debug-rates`). Default `200`, clamped 10–300. Higher = more distinct `mappedRoomId` coverage per hotel (especially luxury hotels where one room can carry 8+ rate plans and crowd out the others in the response), at the cost of payload size and parse time. The `[rates] distinct-rooms histogram` log line is the empirical signal for tuning: if most priced hotels are in the 0–1 bucket we're under-fetching; if most are 6+ we may be over-fetching. At 100 the Mexico City histogram clustered tightly at 1–2/hotel with 0 hotels reaching 6+; bumped to 200 to push more hotels into 4-5/6+ buckets. Above ~250 hits diminishing returns and gets close to LiteAPI's payload ceiling. **Note:** raw coverage from LiteAPI is structurally capped (their `/hotels/rates` sorts by cheapness across the whole hotel, so cheap rooms' rate plans monopolize the response window). The supplementary fix is D3 (`roomNames` + extra-rate rows on the client) which surfaces priced rooms we don't have indexed photos for as "MORE BOOKABLE RATES" thin rows.
```
