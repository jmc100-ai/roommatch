...
3. **Detail pass** (when batch size `>= 50`): re-query the top `RATES_DETAIL_TOPN` (default 50) ranked hotels in parallel chunks of `RATES_DETAIL_CHUNK` (default 15) each, assigned **round-robin** (not sequential). Reason: LiteAPI has a per-request global rate budget that gets split across the cohort. Big batches → 1 rate per hotel; small batches with low-room-count hotels → 3-5 rates each; small batches with high-room-count hotels (e.g. luxury 4-5⭐ with 15-20 room types) split thinly so each only returns 1-2 rates. Adjacent hotels in search-ranked order tend to share a star tier (top hits are luxury, mid-pack is mid-tier, etc.), so sequential chunks co-locate same-tier hotels and they fight for the budget. **Round-robin chunk assignment interleaves star tiers across chunks** so each chunk has a mix of room-count sizes, which empirically lets the smaller hotels keep their full rate counts. Validated against `lp3e1a2` (3 rooms) co-located with `lp4bab4` (St. Regis, 20 rooms): sequential chunking capped lp3e1a2 at 2 rates; round-robin returns all 3. The detail pass merges all 3-5 rates per top hotel into `roomPrices` / `roomNames` / `offerIds` / `roomFreeCancel`. Failure tolerant: `Promise.allSettled` — partial chunk failures just leave those hotels at whatever the main call returned.
