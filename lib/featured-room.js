...
const { factsMeetMustSpec } = require("./must-have-spec");

/** Server-side: room-type facts jsonb meets all required must-have keys (AND). */
...
/** AND of single facts and OR groups — preferred for V2 search. */
function factsMeetMustRequirements(features, spec) {
  return factsMeetMustSpec(features, spec);
}
