...
/**
 * Best Match room signal. When live rates are loaded, use the best vibe score among
 * **priced** indexed room types (what the card can actually book). Falls back to
 * max-all-rooms only when LiteAPI returned a hotel price but no per-room ID overlap.
 */
function bestMatchRoomScore(h, ctx = {}) {
  const allRooms = h?.roomTypes || [];
  if (ctx.pricesLoaded && ctx.hasDateSearch && h?.roomPrices && allRooms.length) {
...
function sortHotelsBestMatch(hotels, stats, profile, ctx) {
  ...
  const roomMatchScore = (h) => bestMatchRoomScore(h, ctx);
  ...
  list.sort((a, b) => {
    ...
    if (Math.abs(pm) <= BOOP_PRICE_NEUTRAL_BAND) {
      ...
      if (roomGap >= domGap && nbhdGap <= BOOP_NBHD_SIMILAR_MAX) return 1;
      if (roomGap <= -domGap && nbhdGap <= BOOP_NBHD_SIMILAR_MAX) return -1;
    }
    if (pm > 0) {
      const roomGap = roomB - roomA;
