...
const MATCH_LIVE_RATE_NUDGE_MAX = 14;
const MATCH_LIVE_RATE_NUDGE_ROOM_GAP = 4;
...
} = require("./featured-room");

function roomVibeMatchDisplayPct(h, ctx = {}) {
  return featuredRoomMatchPct(h, ctx);
}

/** Best Match room signal — always matches the card ROOM VIBE % badge (F1). */
...
function effectiveNbhdWeightForPriceMatters(baseW, pm) {
  ...
  const p = Number(pm) || 0;
  if (p <= 0) return w;
    return Math.min(0.62, w * (1 + BOOP_PRICE_NBHD_WEIGHT_BOOST * (Math.abs(p) / 100)));
}

function shouldNbhdGuardYieldToPrice(weakNbhdHotel, strongNbhdHotel, pm = 0) {
  if (Math.abs(pm) <= BOOP_PRICE_NEUTRAL_BAND) return false;
...
function matchLiveRateNudgeDiff(a, b, roomA, roomB, pricePercentiles) {
  ...
  if (priceRatioToMedian >= MATCH_LIVE_RATE_NUDGE_MIN_RATIO) {
    const ratioBoost =
      MATCH_LIVE_RATE_RATIO_BOOST * Math.min(2.2, priceRatioToMedian / MATCH_LIVE_RATE_NUDGE_MIN_RATIO);
...
function sortHotelsBestMatch(hotels, stats, profile, ctx) {
  ...
  const sortScore = (h) => {
    ...
    if (pmNeutral && pricePercentiles) s -= neutralLivePricePenalty(h, pricePercentiles, pm);
    return s;
  };
  ...
  list.sort((a, b) => {
    ...
    if (Math.abs(pm) <= BOOP_PRICE_NEUTRAL_BAND && !shouldSkipRoomDominance(a, b, profile)) {
      const roomGap = roomB - roomA;
      const domGap = roomDominanceGapForProfile(profile, useNeutralLiveSort);
      if (roomGap >= domGap) return 1;
      if (roomGap <= -domGap) return -1;
    }
    if (pm > BOOP_PRICE_NEUTRAL_BAND) {
      const roomGap = roomB - roomA;
      ...
      if (wNbhd > 0 && nbhdA != null && nbhdB != null) {
        ...
        if (nbhdGap >= BOOP_PRICE_NBHD_GAP_GUARD) {
          if (!shouldNbhdGuardYieldToPrice(a, b, pm)) return 1;
        } else if (nbhdGap <= -BOOP_PRICE_NBHD_GAP_GUARD) {
          if (!shouldNbhdGuardYieldToPrice(b, a, pm)) return -1;
        }
      }
    }
    ...
    if (Math.abs(diff) < 1e-6) diff = (Number(b.hotelScore) || 0) - (Number(a.hotelScore) || 0);
    if (Math.abs(diff) < 1e-6) diff = (Number(b.rating) || Number(b.guestRating) || 0) - (Number(a.rating) || Number(a.guestRating) || 0);
    if (Math.abs(diff) < 1e-6) diff = String(b.id).localeCompare(String(a.id));
...
module.exports = {
  ...
  roomDominanceGapForProfile,
  shouldSkipRoomDominance,
  BOOP_ROOM_DOMINANCE_NBHD_SKIP_GAP,
};
