...
function deriveNbhdSignals(h) {
  ...
  const s = {
    ...
    // Match client deriveNbhdSignals: OSM park polygons over-count medians on boulevards.
    green:       v("parks") * 0.38 + v("greenery") * 0.52 + natureBonus,
    foodie:      v("restaurants") * 0.65 + v("cafes") * 0.35,
    culture:     pRaw("museums") * 0.55 + pRaw("icon_spots") * 0.45,
    ...
    calm:
      ...
      v("restaurants") * 0.02 +
      (streetFeel <= 40 ? 14 : 0),
    central:     pRaw("icon_spots") * 0.55 + v("street_feel") * 0.25 + (textHasAny(txt, ["central", "heart", "iconic"]) ? 14 : 0),
