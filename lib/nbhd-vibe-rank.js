...
async function applyNbhdBoopRank(fetchClient, city, rankedHotels, boopProfile, opts) {
  ...
  // Primary-by-city cache can reference deleted neighborhood ids after a city
  // regen (London Jun 2026: 201 cached rows pointed at ids 238–247 while
  // current hoods are 295+). Stale ids break nbhd_fit scoring and cause search
  // to skip the display RPC — UI falls back to the city name ("London").
  if (primaryByHotel?.size && hoodRows.length) {
    const validNbhdIds = new Set(hoodRows.map((h) => h.id));
    let staleCount = 0;
    for (const nid of primaryByHotel.values()) {
      if (!validNbhdIds.has(nid)) staleCount++;
    }
    if (staleCount > 0) {
      console.log(
        `[nbhd-vibe-rank] stale primary cache for ${city}: ${staleCount}/${primaryByHotel.size} ids invalid — refetching`
      );
  ...
  // Attach nbhd_fit_pct only — do not re-sort rankedHotels here. Final order is
  // applied once in search-v2 (primarySignal + nbhd blend). Re-sorting the full
  // catalog at weight≈0.62–0.76 collapsed leafy_local results to one hood (Condesa).
