...
} = require("./rates-snapshot");

const RATES_CACHE_TTL_MS = Math.max(30_000, Math.min(900_000,
  Number(process.env.RATES_CACHE_TTL_MS)
    || (snapshotEnabled() ? RATES_SNAPSHOT_TTL_MS : 180_000)));
    ...
    for (const rt of (hotel.roomTypes || [])) {
      ...
      if (mappedRoomId) {
        ...
        if (existing == null || perNight < existing) {
          ...
          // offerId lives on the roomTypes/offer row per LiteAPI docs, not on rates[0].
          const offerId = rt.offerId || rt.offer_id || firstRate?.offerId || firstRate?.offer_id;
          if (offerId) {
...
function snapshotPayloadToResult(snapshot, ids, t0) {
  return {
    prices: snapshot.prices || {},
    roomPrices: snapshot.roomPrices || {},
    roomNames: snapshot.roomNames || {},
    offerIds: snapshot.offerIds || {},
    roomFreeCancel: snapshot.roomFreeCancel || {},
    hotelFreeCancel: snapshot.hotelFreeCancel || {},
    currency: snapshot.currency,
    nights: snapshot.nights,
    pricedCount: snapshot.pricedCount || 0,
    skip_detail: snapshot.skip_detail,
    wall_ms: Date.now() - t0,
    hotel_ids_fetched: ids.length,
    cache_hit: true,
 ...
 * @param {{ skipDetail?: boolean, cacheKey?: string, supabase?: object }} opts — skipDetail=true for fast vsearch embed (main pass only).
 */
async function fetchRatesForHotelIds(hotelIds, checkin, checkout, currency = "USD", opts = {}) {
  ...
  if (cacheKey && supabase) {
    const snap = await readRatesSnapshot(supabase, cacheKey);
    if (snap?.pricedCount > 0) {
      const fromSnap = snapshotPayloadToResult(snap, ids, t0);
      setCachedRates(cacheKey, {
        prices: fromSnap.prices,
        roomPrices: fromSnap.roomPrices,
        roomNames: fromSnap.roomNames,
        offerIds: fromSnap.offerIds,
        roomFreeCancel: fromSnap.roomFreeCancel,
        hotelFreeCancel: fromSnap.hotelFreeCancel,
        currency: fromSnap.currency,
        nights: fromSnap.nights,
        pricedCount: fromSnap.pricedCount,
        skip_detail: fromSnap.skip_detail,
      });
      return fromSnap;
    }
  }
  ...
  if (cacheKey && result.pricedCount > 0) {
    ...
    setCachedRates(cacheKey, cachePayload);
    if (supabase) {
      writeRatesSnapshot(supabase, cacheKey, result).catch((e) => {
        console.warn("[rates-snapshot] write failed:", e.message);
      });
    }
  }
