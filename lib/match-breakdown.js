...
function buildMatchBreakdown(p) {
  ...
  const spec = mustHaveSpec?.length
    ? mustHaveSpec
    : [...new Set([...mustHaveKeys, ...hardFilterKeys].filter(Boolean))].map((k) => k);
  const roomFacts = featuredRoomFacts || null;
  const requirements = mustHaveSpec?.length
    ? mustHaveRequirementsFromSpec(spec)
    : spec.map((fact) => ({ id: fact, fact, label: humanizeFactKey(fact) }));

  const must_haves = requirements.map((req) => {
    const reqKey = req.orFacts || req.fact || req.id;
    let status = "none";
    let matched_fact = null;
    if (roomFacts) {
      const met = isOrGroup(reqKey)
        ? requirementMet(roomFacts, reqKey)
        : roomFacts[reqKey] === true;
      if (met) {
        status = "met";
        matched_fact = isOrGroup(reqKey)
          ? firstMatchingOrFact(roomFacts, reqKey)
          : reqKey;
      }
    } else if (featuredRoom != null) {
      status = featuredRoom.must_haves_met ? "met" : "none";
      if (featuredRoom.must_haves_met) {
        if (isOrGroup(reqKey)) {
          matched_fact = firstMatchingOrFact(
            Object.fromEntries([...flatHitFacts(hits)]),
            reqKey
          );
        } else {
          matched_fact = req.fact || req.id;
        }
      }
    } else if (!isOrGroup(reqKey) && hits?.has(reqKey)) {
      status = "met";
      matched_fact = reqKey;
    }
    return {
      id: req.id,
      fact_key: matched_fact || (isOrGroup(reqKey) ? "balcony_or_view" : reqKey),
      label: req.label || humanizeFactKey(reqKey),
      status,
      matched_fact,
    };
  });
  ...
  const flatMustFacts = flatFactsFromSpec(spec);
  const queryOnlyKeys = detectedFactKeys.filter((fk) => fk && !flatMustFacts.includes(fk));
  ...
  return {
    ...
    hotel_pct: hotelPct,
    nbhd_pct: nbhdPct,
    must_haves_summary: requirements.length
      ? { met: metCount, total: requirements.length }
      : null,
    must_haves,
    query_features,
...
function flatHitFacts(hits) {
  if (!hits) return [];
  return [...hits].map((k) => [k, true]);
}

function flatFactsFromSpec(spec) {
  const out = [];
